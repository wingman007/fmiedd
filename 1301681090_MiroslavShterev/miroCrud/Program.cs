...
namespace CRUDusers
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.Clear();
            aConnection = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=E:\m\miroCrud\miroCrud\data\users.accdb");
                ...
                while (true)
                {
                    ...
                    switch (action)
                    {
                        ...
                        case 2:
                            ...
                            {
                                Console.WriteLine("You don't have permission modify");
                            }
                        ...
                        case 3:
                            if (role == 1)
                            {
                                Console.WriteLine("Which line you want to delete? ");
                                try
                                {
                            ...
                            {
                                Console.WriteLine("You don't have permission to modify");
                            }
        ...
        {
            OleDbCommand selectUser = new OleDbCommand("SELECT COUNT(*) FROM users WHERE username=@user AND password=@pass");
            selectUser.Parameters.AddRange(new[] { new OleDbParameter("@user", username) });
            selectUser.Parameters.AddRange(new[] { new OleDbParameter("@pass", password) });
            aConnection.Open();
        ...
        public static int getRole(string username, string password)
        {
            int role_id = 0;
            aConnection.Open();
            OleDbCommand selectUser = new OleDbCommand("SELECT * FROM users WHERE username=@user AND password=@pass", aConnection);
            selectUser.Parameters.AddRange(new[] { new OleDbParameter("@user", username) });
            selectUser.Parameters.AddRange(new[] { new OleDbParameter("@passss", password) });
