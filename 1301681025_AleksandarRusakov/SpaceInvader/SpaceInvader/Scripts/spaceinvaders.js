...
function Game() {
    ...
    this.config = {
        ...
        rocketVelocity: 120,
        rocketMaxFireRate: 2,
        gameWidth: 400,// Size of the level
        gameHeight: 350,
        fps: 50,// Speed of the game
        debugMode: false,
        invaderRanks: 5,
        invaderFiles: 10,
        shipSpeed: 120,
...
// Move to a state of the game
Game.prototype.moveToState = function(state) {

    // See if we are in a state alreay
    // See if the state hase a leave function
    //  If we are in a state, leave it.
    if(this.currentState() && this.currentState().leave) {
        // If true we call it
        this.currentState().leave(game);// To exit
        this.stateStack.pop();// Pop(Remove) the current state
    }
...
Game.prototype.currentState = function() {
        // If stateStack is not empty return the last element else returs null
...
function PlayState(config, level) {
    ...
    // How many frame will have the sprite animation
    //this.invaderNumberOfFrames = 2;

    // How many frames of game for next sprite frame
    //  The number updates until the next frame should be displayed
    //this.invaderTicksPerFrame = 10;
    this.invaderNumberOfFrames = 2;
    this.invaderTicksPerFrame = 10;
...
PlayState.prototype.enter = function(game) {
    ...
    // Create the ship
    // To start in center and be on the bottom
    this.ship = new Ship(game.images, game.width / 2, game.gameBounds.bottom);
    ...
    for(var rank = 0; rank < ranks; rank++){
        for(var file = 0; file < files; file++) {
            invaders.push(new Invader(
                    game.images,
                    (game.width / 2) + ((files/2 - file) * 275 / files),
...
PlayState.prototype.update = function(game, dt) {
    ...
    for (var i = 0; i < this.invaders.length; i++) {// Fro every invader

        var invader = this.invaders[i];

        invader.tickCount += 1;
        if (invader.tickCount > this.invaderTicksPerFrame) {

            invader.tickCount = 0;

            if (invader.frameIndex < this.invaderNumberOfFrames - 1) {
                invader.frameIndex += 1;
                invader.animation += 16;
            } else {
                invader.frameIndex = 0;
                invader.animation = 0;
            }
        }
    }
    ...
    // !!! COLLISION ON BOMBS AND SHIP !!!
    for(var i=0; i<this.bombs.length; i++) {
        var bomb = this.bombs[i];
        if(bomb.x >= (this.ship.x - this.ship.width/2) &&
...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    for(var i=0; i<this.rockets.length; i++) {
        var rocket = this.rockets[i];
        ctx.fillRect(rocket.x, rocket.y - 2, 2, 4);
    }
...
PlayState.prototype.fireRocket = function() {
    ...
    {
        //  Add a rocket.
        this.rockets.push(new Rocket(this.ship.x, this.ship.y - 2, this.config.rocketVelocity));
...
function Invader(img, x, y, rank, file, type, animation, frameIndex, tickCount, numberOfFrames, ticksPerFrame) {
    this.img = img;
    this.x = x;
    this.y = y;
    this.rank = rank;
    ...
    // Set the new frame in the sprite position
    // X or Y position in pixels
    // Set the way is created in the sprite sheet
    this.animation = animation;

    // Count how many frames where seen (must be 0)
    //   The current frame to be displayed
    this.frameIndex = frameIndex;

    // Count how many tickes will have for the next frame (must be 0)
